...
class NeuralRadianceField(torch.nn.Module):
    ...
    def forward(
        ...
        ray_dir_norm = torch.nn.functional.normalize(
            ray_bundle.directions,
            dim=-1
        )
        pts_embeddings = self.harmonic_embedding_xyz(pts)
        ray_dir_embeddings = self.harmonic_embedding_dir(ray_dir_norm)
        # print("ray dir embedding shape: ", ray_dir_embeddings.shape)
        #feature encoding
        feat = self.encoder_layer1(pts_embeddings)
        
        feat = self.encoder_layer2(feat)
        densities = self.density_head(feat)
        # print("points embedding: ", torch.unique(pts_embeddings))
        # print("\nfeat value: ", torch.unique(feat))
        # print("\ndensity value: ", torch.unique(densities))
        feat_pre_dim = feat.shape[:-1]
        # print("feat pre dim: ", feat_pre_dim)
        ray_dir_embeddings_expand = ray_dir_embeddings[..., None,:].expand(
            *feat_pre_dim, ray_dir_embeddings.shape[-1]
        )
        # print("ray dir embedding shape: ", ray_dir_embeddings_expand.shape)
        color_layer_input = torch.cat(
            (feat, ray_dir_embeddings_expand),
            dim=-1
        )
...
class NeuralSurface(torch.nn.Module):
    def __init__(
        ...
        self.harmonic_embedding_dists = HarmonicEmbedding(3, cfg.n_harmonic_functions_xyz)
        embeddings_dist_dim = self.harmonic_embedding_dists.output_dim
        self.distance_mlp = MLPWithInputSkips(cfg.n_layers_distance, embeddings_dist_dim, embeddings_dist_dim, cfg.n_hidden_neurons_distance, cfg.append_distance)
        self.distance_final = torch.nn.Sequential(torch.nn.Linear(cfg.n_hidden_neurons_distance, 1))

        self.color_mlp = MLPWithInputSkips(cfg.n_layers_color,embeddings_dist_dim,embeddings_dist_dim,cfg.n_hidden_neurons_color,cfg.append_color)
        self.color_final = torch.nn.Sequential(
    ...
    def get_distance(
        ...
        '''
        points = points.view(-1, 3)
        emp = self.harmonic_embedding_dists(points)
    ...
    def get_color(
        ...
        points = points.view(-1, 3)
        emp = self.harmonic_embedding_dists(points)
        o = self.color_mlp(emp,emp)
        rgb = self.color_final(o)
        return rgb
    ...
    def get_distance_color(
        ...
        return self.get_distance(points=points ), self.get_color(points=points)
