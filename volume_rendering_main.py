...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        # print("after get bundle")
        # TODO (Q1.4): Visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            pass
            # pts = ray_bundle.sample_points
            # render_points("images/vis.jpg", torch.reshape(pts, (1, pts.shape[0] * pts.shape[1], 3)))

        # TODO (Q1.5): Implement rendering in renderer.py
        out = model(ray_bundle)
        # print("out feature shape: ", out['feature'].shape)
        # print("out depth shape: ", out['depth'].shape)
        # Return rendered features (colors)
        image = np.array(
            out['fine_feature'].view(
        ...
        if cam_idx == 2 and file_prefix == '':
            # print("unique depth: ", torch.unique(out['depth']))
...
def create_model(cfg):
    ...
    if len(cfg.training.checkpoint_path) > 0:
        ...
        if cfg.training.resume and os.path.isfile(checkpoint_path) and False:
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            loss =  torch.nn.functional.mse_loss(out["fine_feature"], rgb_gt) 
            + torch.nn.functional.mse_loss(out["feature"], rgb_gt)
        ...
        if (
            ...
            with torch.no_grad():
                ...
                imageio.mimsave('images/part_3_refine.gif', [np.uint8(im * 255) for im in test_images])
